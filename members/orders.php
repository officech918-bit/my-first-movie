 ...
 *
 * This script processes the encrypted response from the CCAvenue payment gateway,
 * decrypts it, validates the transaction, creates order records in the database,
 * and sends confirmation emails to the customer.
 *
 * @package MFM
 * @subpackage Members
 */
...
if(isset($_POST["encResp"])) {	
	...
	// Check if the record has already been added to the database to prevent double insertion.
	$stmt = $database->db->prepare("SELECT 1 FROM orders WHERE tracking_id = ?");
	$stmt->execute([$tracking_id]);
	$is_first_process = (bool) $stmt->fetchColumn();
	$stmt->closeCursor();


	if (!$is_first_process) {
		// Create the order using a prepared statement.
		$query = "INSERT INTO orders(user_id, date, ip_address, order_id, product_amount,  service_tax, amount, currency, billing_name, billing_email, billing_tel, billing_address, billing_city, billing_state, billing_zip, billing_country, i_agree, tracking_id, bank_ref_no, order_status, failure_message, payment_mode, card_name, status_code, status_message, vault) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$stmt = $database->db->prepare($query);
		$stmt->execute([
			$user_id, $date, $ip, $order_id, $product_amount, $service_tax, $amount, $currency,
			$billing_name, $billing_email, $billing_tel, $billing_address, $billing_city,
			$billing_state, $billing_zip, $billing_country, $i_agree, $tracking_id,
			$bank_ref_no, $order_status, $failure_message, $payment_mode, $card_name,
			$status_code, $status_message, $vault
		]);
		$stmt->closeCursor();
		...
		if (isset($_SESSION["subscriptions"])) {
			foreach ($_SESSION["subscriptions"] as $cart_itm) {
				...
				$stmt = $database->db->prepare("SELECT duration FROM pricing WHERE price_id = ?");
				$stmt->execute([$subscription_price_id]);
				$duration = $stmt->fetchColumn() ?? 'Single Issue';
				$stmt->closeCursor();

				$expire_date = '';
				if ($duration === 'Single Issue') { $expire_date = date('Y-m-d H:i:s', strtotime("+7 day")); }
				elseif ($duration === '3 Months') { $expire_date = date('Y-m-d H:i:s', strtotime("+3 months")); }
				elseif ($duration === '6 Months') { $expire_date = date('Y-m-d H:i:s', strtotime("+6 months")); }
				elseif ($duration === '1 Year') { $expire_date = date('Y-m-d H:i:s', strtotime("+1 year")); }
				elseif ($duration === '2 Year') { $expire_date = date('Y-m-d H:i:s', strtotime("+2 year")); }

				$stmt = $database->db->prepare("INSERT INTO orders_subscription(order_id, subscription_id, price_id, user_id, pricing_slab, subscription_from, expire_date) VALUES (?, ?, ?, ?, ?, ?, ?)");
				$stmt->execute([$order_id, $subscription_id, $subscription_price_id, $user_id, $duration, $date, $expire_date]);
				$stmt->closeCursor();
			}
		}
	...
	if($order_status==="Success")
	{
		...
		$message = '<html>
	<head>
	<title>Project Alert Order #'.$order_id.'</title>
	</head>
				...
				<tbody>
				  <tr>
					<td align="center"><table style="width: 750px;" border="0" cellspacing="0" cellpadding="0" align="center" bgcolor="#f5f5f5">
						<tbody>
						  <tr>
							<td width="10%" height="65" align="left"><div style="min-height: 45px;"><a href="http://projectalert.in/" target="_blank"><img src="'.$sitename.'assets/frontend/layout/img/logos/logo.png" border="0" alt="" width="227" height="50" /></a></div></td>
							<td width="90%" align="right" valign="bottom"><span style="font-size: 40px; font-family:Century Gothic; color: #666666;">Order<br />
							  ...
							  <div><span style="font-size: 11px; line-height: 16px; color: #999999;font-family:Century Gothic;"> '.$billing_name.', this is the order you have placed.<br />
								</span></div></td>
						  </tr>
						</tbody>
					  </table>
									...
									$sub_total = 0;
										  ...
										  if (isset($_SESSION["subscriptions"])) {
											  foreach ($_SESSION["subscriptions"] as $cart_itm) {
												  ...
												  $stmt = $database->db->prepare("SELECT title FROM subscriptions WHERE subcription_id = ?");
												  ...
												  if ($subscription_data) {
													  // Get pricing details
													  $stmt = $database->db->prepare("SELECT price_id, duration, amount FROM pricing WHERE subscription_id = ?");
													  $stmt->execute([$subscription_id]);
													  $pricing_result = $stmt->fetchAll(PDO::FETCH_ASSOC);
										  
													  foreach ($pricing_result as $row) {
														  if ((int)$row['price_id'] === $subscription_price_id) {
															  $amount = (float) $row['amount'];
															  $sub_total += $amount;
															  $subscription_rows_html .= '<tr><td style="padding:6px;border-left:1px solid #bcbcbc;border-bottom:1px solid #bcbcbc" valign="middle">' . htmlspecialchars($subscription_data['title']) . '</td><td style="padding:6px;border-left:1px solid #bcbcbc;border-bottom:1px solid #bcbcbc" valign="middle">' . htmlspecialchars($row['duration']) . '</td><td style="padding:6px;border-left:1px solid #bcbcbc;border-bottom:1px solid #bcbcbc;border-right:1px solid #bcbcbc;" align="center">Rs. ' . htmlspecialchars(number_format($amount, 2)) . '</td></tr>';
														  }
													  }
													  $stmt->closeCursor();
												  }
										  ...
										  $message .= $subscription_rows_html;

										$service_tax_raw = $sub_total * 0.14;
										$total_amount_raw = $sub_total + $service_tax_raw;

										$sub_total = number_format($sub_total, 2, '.', '');
										$service_tax = number_format($service_tax_raw, 2, '.', '');
                    ...
                    <table id="report" class="bordered">
                      ...
                      <?php 
					    $user_id = $_SESSION['uid'];
					  	$result3 = mysql_query("SELECT *, DATE_FORMAT(date,'%d/%m/%Y') AS date FROM orders WHERE user_id='$user_id' ORDER BY orders.date DESC");
						...
						else echo "<tr><td colspan='5' class='checkout-model' style='color:#333; line-height: 30px;'>You did't have placed any order! &nbsp;&nbsp;&nbsp; <a class='btn btn-primary' href='".$path."esubscriptions/'  style='color:#FFF; float:none;'>Shop Now</a></td></tr>";
...
<?php 
//destroy cart details, all session data, all post data recived from payment gateway
